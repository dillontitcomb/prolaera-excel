...
//Sheet types: Complete (all possible tables included), Cycle, Annual (only years), any category name (individual report for that category));
...
function createWorksheet(
  ...
  const {
    ...
    tableSummaryData,
    richTextData
  } = data.buildReportData(profile, regulators[0], certificates, reportType);

  // add rows to worksheet

  let allRows = headerData[0];

  for (let i = 0; i < tableBodyData.length; i++) {
    allRows = allRows
      .concat(subHeaderData[i])
      .concat(tableHeaderData[i])
      .concat(tableBodyData[i])
      .concat(tableSummaryData[i]);
  }
  ...
  for (let i = 0; i < tableBodyData[0].length - 1; i++) {
    let rowNum = 10 + i;
  ...
  let summaryStart = 10 + tableBodyData[0].length;
  for (let i = 0; i < 10; i++) {
